...
def decode_image(image_data, palette_data, image_format, palette_format, num_colors, image_width, image_height):
  ...
  pixels = [None] * image_width * image_height#image.load()
  offset = 0
  block_x = 0
  block_y = 0
...
def decode_cmpr_block(image_format, image_data, offset, block_data_size, colors, out):
  ...
  for subblock_index in range(4):
    ...
    color_0_rgb565 = image_data.readUInt16()
    color_1_rgb565 = image_data.readUInt16()
    colors = get_interpolated_cmpr_colors(color_0_rgb565, color_1_rgb565)
    
    color_indexes = image_data.readUInt32()
    for i in range(16):
...
def encode_image_to_cmpr_block(pixels, colors_to_color_indexes, block_x, block_y, block_width, block_height, image_width, image_height):
  ...
  for subblock_index in range(4):
    ...
    new_data.writeUInt16(color_0_rgb565)
    new_data.writeUInt16(color_1_rgb565)
    ...
    new_data.writeUInt32(color_indexes)
    
    subblock_offset += 8
  
  new_data.seek(0)
  return new_data.fhandle.read()
